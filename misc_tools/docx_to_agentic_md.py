...
def classify_paragraph(paragraph: ET.Element, styles: Dict[str, StyleInfo], image_counter: int) -> Tuple[List[ParaToken], int]:
    ...
    # Gather run-level facts for code detection.
    ...
    if all_courier:
        # Treat fully red Courier paragraphs as commands; all others as code responses.
        has_red = False
        has_non_red = False
        for txt, fmt in non_ws_runs:
            if is_red(fmt.color):
                has_red = True
            else:
                has_non_red = True
...
def flush_response_block(out: List[str], response_lines: List[str]) -> None:
    if not response_lines:
        return
    # Preserve every line exactly as collected.
    block = "\n".join(response_lines)
    out.append("```text")
    out.append(block)
    out.append("```")
...
def emit_markdown(tokens: List[ParaToken]) -> str:
    ...
    for tok in tokens:
        ...
        if tok.kind == "blank":
            # blank lines should be preserved in prose flow, but not multiplied excessively
            if out and out[-1] != "":
                out.append("")
        ...
        if tok.kind == "command":
            ensure_paragraph_spacing()
            # Keep command text exactly, without code fencing.
            out.append(tok.text)
            out.append("")
            continue
...
def convert_docx_to_markdown(docx_path: Path, output_path: Path) -> None:
    ...
    try:
        ...
        for block in reader.iter_block_items():
            # Only paragraphs are handled explicitly. Tables are flattened cell-by-cell.
            if block.tag == f"{W}p":
                ptoks, image_counter = classify_paragraph(block, reader.styles, image_counter)
                tokens.extend(ptoks)
            elif block.tag == f"{W}tbl":
                # Best-effort table flattening: emit rows as pipe-separated text paragraphs.
